...
//                     return {
//                         success: true,
//                         message: response.message || 'Logo removed successfully',
//                         data: response
//                     };
//                 }
...
//                     return {
//                         success: false,
//                         message: 'Logo not found for this location'
//                     };
//                 }
...
//                 const response = await apiService.getFontSettings(state.currentLocation.locationId);
//                 const fontData = response.data || response.font;

//                 if (fontData) {
//                     this.applyFontCSS({
//                         fontFamily: fontData.fontFamily,
//                         headingSize: fontData.headingSize,
//                         contentSize: fontData.contentSize
//                     });
//                 }
//             } catch (error) {
//                 console.error('Error loading font settings:', error);
...
//         async handleLogoUpload() {
//             const fileInput = document.getElementById('logo-upload');
//             const uploadBtn = document.querySelector('button[onclick="logoService.handleLogoUpload()"]');

//             if (!fileInput || !fileInput.files?.length) {
//                 uiService.showNotification('Please select a logo file to upload.', 'warning');
//                 return;
//             }
...
//             if (!file.type.startsWith('image/')) {
//                 uiService.showNotification('Please select a valid image file (PNG, JPG, JPEG, SVG).', 'error');
//                 return;
//             }

//             if (file.size > 5 * 1024 * 1024) {
//                 uiService.showNotification('Image size must be less than 5MB.', 'error');
//                 return;
//             }
...
//                 formData.append("locationId", state.currentLocation.locationId);
//             } else {
//                 uiService.showNotification('Cannot determine current location. Please refresh the page.', 'error');
//                 this.resetUploadButton(uploadBtn);
//                 return;
//             }

//             try {
//                 uiService.showNotification('Uploading logo...', 'info');
//                 const result = await apiService.uploadLogo(formData);

//                 let logoUrl = null;

//                 if (result.logos && result.logos.logo) {
//                     logoUrl = result.logos.logo;
//                 } else if (result.logo) {
//                     logoUrl = result.logo;
//                 } else if (result.data && result.data.logo) {
...
//                 if (uploadBtn) {
//                     uploadBtn.innerHTML = '<i class="fas fa-check"></i> Logo Uploaded!';
//                     uploadBtn.style.background = '#10B981';
...
//                     }, 3000);
//                 }
//                 uiService.showNotification('Logo uploaded successfully!', 'success');
//                 fileInput.value = '';
...
//                     if (uploadBtn) {
//                         uploadBtn.innerHTML = '<i class="fas fa-check"></i> Logo Applied!';
//                         uploadBtn.style.background = '#10B981';
...
//                     if (error.message.includes('Missing locationId') || error.message.includes('400')) {
//                         uiService.showNotification('Logo applied locally (backend configuration issue)', 'info');
//                     } else if (error.message.includes('Invalid JSON response') || error.message.includes('500') || error.message.includes('public_id')) {
//                         uiService.showNotification('Logo applied locally (backend temporarily unavailable)', 'info');
//                     } else {
//                         uiService.showNotification('Logo applied locally!', 'info');
//                     }
//                 } else {
//                     uiService.showNotification('Failed to upload logo. Please try again.', 'error');
//                 }
...
//                 button.innerHTML = 'Upload & Apply Logo';
...
//                 return false;
//             } catch (error) {
//                 console.error('Error loading local logo:', error);
//                 return false;
//             }
//         },
...
//                         uiService.showNotification('Logo removed locally (backend endpoint not configured)', 'info');
//                     } else if (error.message.includes('public_id')) {
//                         uiService.showNotification('Logo removed locally (filename issue resolved)', 'info');
//                     } else {
//                         uiService.showNotification('Logo removed locally (backend temporarily unavailable)', 'info');
//                     }

//                     return true;
//                 } catch (localError) {
//                     console.error('Error removing logo locally:', localError);
//                     uiService.showNotification('Error removing logo: ' + error.message, 'error');
//                     return false;
//                 }
//             }
//         },
...
//         mountBeforeHeaderIcons() {
//             if (location.pathname.includes("/page-builder/")) {
//                 return false; // page builder has no header; skip mounting
//             }
...
//             // Reset retry count if we successfully mounted before
//             if (document.getElementById(CONFIG.BTN_ID)) {
//                 state.mountRetryCount = 0;
//                 return true;
//             }
...
//             if (state.mountRetryCount >= state.MAX_MOUNT_RETRIES) {
//                 console.warn('Max mount retries reached, giving up');
//                 return false;
//             }
...
//     // Enhanced header containers list for page-builder pages
// let targetContainer = document.querySelector('.hl_header--controls');
// if (!targetContainer) return false;
...
// // Find the AI Copilot icon
// const copilotIcon = document.querySelector('#hl_header--copilot-icon');

// // Insert BEFORE Copilot (this becomes the very first icon)
// if (copilotIcon) {
//     targetContainer.insertBefore(btn, copilotIcon);
// } else {
//     targetContainer.prepend(btn); // fallback
// }
...
//             const locationInfo = state.currentLocation
//                 ? `${state.currentLocation.isAgency ? '🏢 Agency' : '📍 Location'}: ${state.currentLocation.name}`
...
//                         <div style="display:flex;gap:8px;align-items:center;">
//                             ${state.currentTheme ? `
//                                 <button id="btn-remove" class="btn btn-reset" style="padding:8px 12px; font-size:12px;">Remove Theme</button>
//                             ` : ''}
...
//             if (confirm(`Are you sure you want to remove the current theme from ${state.currentLocation?.name || 'this location'}?`)) {
//                 themeManager.removeTheme()
//                     .then(() => {
//                         this.showNotification('Theme removed successfully!', 'success');
//                         cacheService.clearCache();
...
//             [fontSelect, headingSize, contentSize].forEach(el => {
//                 if (el) {
//                     el.addEventListener('input', () => {
//                         fontService.applyFontCSS({
//                             fontFamily: fontSelect.value,
//                             headingSize: headingSize.value,
//                             contentSize: contentSize.value
//                         });
...
//             },
//             logo() {
//                 setTimeout(() => uiService.initLogoUploadListener(), 50);
//                 return `
//                         <h2>Logo Settings</h2>
//                         <p>Upload your custom logo to replace the default GHL logo.</p>
//                         <div style="display:flex; flex-direction:column; gap:16px;">
//                             <div>
//                                 <label for="logo-upload" style="font-weight:600; margin-bottom:6px; display:block;">Upload Logo</label>
//                                 <input type="file" id="logo-upload" accept="image/*" style="padding:6px; border-radius:8px;">
//                                 <small style="color:var(--text-secondary); font-size:12px; display:block; margin-top:4px;">
//                                     Logo will be displayed at optimal medium size (120px width)
//                                 </small>
//                             </div>
                            
//                             <button onclick="logoService.handleLogoUpload()" style="background:#9CA3AF; color:#fff; padding:12px 16px; border:none; border-radius:10px; font-weight:600; cursor:not-allowed; transition: all 0.3s ease; display:flex; align-items:center; justify-content:center; gap:8px;" disabled>
//                                 <i class="fas fa-upload"></i>
//                                 Select a file to upload
//                             </button>
//                             <button onclick="logoService.removeLogo()" style="background:#DC2626; color:#fff; padding:12px 16px; border:none; border-radius:10px; font-weight:600; cursor:pointer; margin-top:8px; display:flex; align-items:center; justify-content:center; gap:8px;">
//                                 <i class="fas fa-trash"></i>
//                                 Remove Logo
//                             </button>
//                         </div>
...
//             },
//             font() {
//                 setTimeout(() => uiService.initFontControls(), 50);
//                 return `
//                         <h2>Brand Font</h2>
//                         <p>Customize the font family and sizes across your GHL dashboard.</p>
//                         <div style="display:flex; flex-direction:column; gap:16px;">
//                             <div>
//                                 <label for="font-family-select" style="font-weight:600; margin-bottom:6px; display:block;">Font Family</label>
//                                 <select id="font-family-select" style="width:100%; padding:12px; border-radius:8px;">
//                                     <option value="Arial">Arial</option>
//                                     <option value="Helvetica">Helvetica</option>
//                                     <option value="Georgia">Georgia</option>
//                                     <option value="Times New Roman">Times New Roman</option>
...
//                             </div>
//                             <div>
//                                 <label for="heading-size" style="font-weight:600; margin-bottom:6px; display:block;">Heading Size</label>
//                                 <input type="number" id="heading-size" value="24" min="10" max="72" style="width:100%; padding:12px; border-radius:8px;">
//                                 <small style="color:var(--text-secondary); font-size:12px;">Applies to h1–h6 elements</small>
//                             </div>
//                             <div>
//                                 <label for="content-size" style="font-weight:600; margin-bottom:6px; display:block;">Content Size</label>
//                                 <input type="number" id="content-size" value="16" min="10" max="48" style="width:100%; padding:12px; border-radius:8px;">
//                                 <small style="color:var(--text-secondary); font-size:12px;">Applies to p, span, div, a elements</small>
//                             </div>
//                             <button onclick="fontService.applyFontSettings({
//                                 fontFamily: document.getElementById('font-family-select').value,
//                                 headingSize: document.getElementById('heading-size').value,
//                                 contentSize: document.getElementById('content-size').value
//                             })" style="background:#111827; color:#fff; padding:12px 16px; border:none; border-radius:10px; font-weight:600; cursor:pointer;">
//                                 Apply Font Settings
//                             </button>
//                         </div>
...
//                 return `
//                         <h2>Brand Colors</h2>
//                         <p>Customize the color scheme of your GHL dashboard.</p>
//                         <div style="display:flex; flex-direction:column; gap:16px;">
//                             <div>
//                                 <label for="sidebar-color" style="font-weight:600; margin-bottom:6px; display:block;">Sidebar Color</label>
//                                 <input type="color" id="sidebar-color" value="#4f46e5" style="width:100%; height:44px; border-radius:8px; cursor:pointer;">
...
//         console.log('Initializing GHL Theme Customizer...');
...
//                         fontService.loadCurrentFont().catch(err => console.warn('Font load warning:', err)),
//                         brandColorsService.loadCurrentColors().catch(err => console.warn('Colors load warning:', err)),
//                         logoService.getAndApplyLogo().catch(err => console.warn('Logo load warning:', err))
//                     ]);
//                 }
//             }, 100);
...
//         state.mountRetryCount = 0;
//         initializeThemeCustomizer();
//         tryMountHeader(); // ensure header mount after navigation
//     }
...
//             if (container) {
//                 uiService.mountBeforeHeaderIcons(); // your existing mount logic
//                 console.log('Theme Builder mounted in', selector);
//                 return;
//             }
//         }
